...
const QuizScreen = () => {
  ...
  const choices =
    currentQuestionData?.[currentLang]?.choices ||
    currentQuestionData?.eng?.choices ||
    [];
      ...
      {gameState === "playing" && (
        <div className="w-11/12 max-w-sm bg-[#381111] rounded-[1.5rem] shadow-2xl animate-fade-in-up mt-8 overflow-hidden flex flex-col border border-white/5">
          ...
          <div className="pt-6 pb-8 px-6 flex flex-col items-center text-center">
            ...
            <h3
              className={`${isCJK ? "font-sans font-bold" : "font-serif"} text-[#FDFBF7] text-[1.35rem] leading-snug min-h-[60px] flex items-center justify-center`}
            >
              {questionText}
            </h3>
          </div>
          ...
          <div className="bg-[#E0CCB6] px-6 py-8 flex flex-col gap-4 border-t-2 border-[#2A0C0C]">
            {choices.map((choice, index) => {
              let buttonStyle =
                "bg-[#D4C3A3] border border-[#4A260F] text-[#4A260F] shadow-[0_3px_0_#4A260F] active:shadow-none active:translate-y-[3px]";

              if (selectedAnswer !== null) {
                if (index === correctIndex) {
                  buttonStyle =
                    "bg-[#4C8C5C] border border-[#1B4B18] text-white shadow-[0_3px_0_#1B4B18] translate-y-0";
                } else if (index === selectedAnswer) {
                  buttonStyle =
                    "bg-[#A35252] border border-[#5A2020] text-white shadow-[0_3px_0_#5A2020] translate-y-[3px] shadow-none";
                } else {
                  buttonStyle =
                    "bg-[#D4C3A3]/50 border border-[#4A260F]/50 text-[#4A260F]/50 shadow-none translate-y-[3px]";
                }
              }
        ...
        <div className="w-full flex flex-col items-center animate-fade-in-up">
          <div className="w-10/12 max-w-[300px] bg-[#381111] p-3 rounded-[1.5rem] shadow-2xl relative border border-white/5">
            <div className="bg-[#E0CCB6] rounded-xl pt-6 pb-8 px-6 flex flex-col items-center text-center border border-[#C4AB8F]">
              <h3
                className={`${isCJK ? "font-sans font-bold" : "font-serif"} text-[#4A260F] text-2xl mb-2`}
              >
              ...
              <div className="w-28 h-28 bg-white rounded-full mb-4 border-[6px] border-[#E6BA39] shadow-md"></div>

              <p
                className={`${isCJK ? "font-sans font-bold" : "font-serif"} text-[#783713] text-xl`}
              >
                Paint Explorer
              </p>
            </div>
