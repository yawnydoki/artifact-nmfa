...
const AdminDashboard = () => {
  ...
  const handleQuizChange = (qNum, lang, field, value, choiceIndex = null) => {
    setEditItem(prev => {
      ...
      const langData = qData[lang] || { question: "", choices: ["", "", "", ""], correct_index: 0 };
      
      if (field === 'choices') {
        const newChoices = [...langData.choices];
        newChoices[choiceIndex] = value;
        return { ...prev, [qNum]: { ...qData, [lang]: { ...langData, choices: newChoices } } };
      } else {
        return { ...prev, [qNum]: { ...qData, [lang]: { ...langData, [field]: value } } };
      }
    });
  };
  ...
  const saveChanges = async () => {
    ...
    try {
      if (isNewItem) {
        ...
        setArtworks(artworks.map(a => a.id === editItem.id ? editItem : a));
                    ...
                    <table className="w-full text-left border-collapse min-w-[800px]">
                      <thead className="sticky top-0 bg-[#E0CCB6] z-10 shadow-sm">
                        <tr className="text-[#4A260F] text-sm uppercase tracking-wider">
                          <th className="p-4 border-b border-gray-300 font-bold">ID</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Image</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Title (EN)</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Artist</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Zone</th>
                          <th className="p-4 border-b border-gray-300 font-bold text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="text-sm">
                        {currentItems.map((art) => (
                          <tr key={art.id} className="hover:bg-gray-50 transition-colors group">
                            <td className="p-4 border-b border-gray-100 text-gray-500">{art.id}</td>
                            <td className="p-4 border-b border-gray-100">
                              {art.thumbnail_url ? (
                                <img src={art.thumbnail_url} alt="thumb" className="w-12 h-12 rounded object-cover border border-gray-200" />
                            ...
                            <td className="p-4 border-b border-gray-100 text-gray-600">{art.artist?.eng || 'Unknown'} {art.artist_year ? `(${art.artist_year})` : ''}</td>
                            <td className="p-4 border-b border-gray-100">
                ...
                {isLoadingData ? (
                  ...
                  <div className="overflow-auto flex-1 rounded-xl border border-gray-200 hide-scrollbar shadow-inner relative">
                    <table className="w-full text-center border-collapse min-w-[800px]">
                      <thead className="sticky top-0 bg-[#E0CCB6] z-10 shadow-sm">
                        <tr className="text-[#4A260F] text-sm uppercase tracking-wider">
                          <th className="p-4 border-b border-gray-300 font-bold text-left">Artwork (EN)</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Tagalog</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Chinese</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Japanese</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Korean</th>
                          <th className="p-4 border-b border-gray-300 font-bold text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="text-sm">
                        {currentItems.map((art) => (
                          <tr key={art.id} className="hover:bg-gray-50 transition-colors group">
                            <td className="p-4 border-b border-gray-100 font-serif font-bold text-[#381111] text-left">{art.title?.eng || 'Untitled'}</td>
                            <td className="p-4 border-b border-gray-100">{hasTranslation(art.title, 'tag')}</td>
                            <td className="p-4 border-b border-gray-100">{hasTranslation(art.title, 'chi')}</td>
                ...
                {isLoadingData ? (
                  ...
                  <div className="overflow-auto flex-1 rounded-xl border border-gray-200 hide-scrollbar shadow-inner relative">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                      <thead className="sticky top-0 bg-[#E0CCB6] z-10 shadow-sm">
                        <tr className="text-[#4A260F] text-sm uppercase tracking-wider">
                          <th className="p-4 border-b border-gray-300 font-bold">Artwork (EN)</th>
                          <th className="p-4 border-b border-gray-300 font-bold text-center">Questions Set</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Sample Q1 (EN)</th>
                          <th className="p-4 border-b border-gray-300 font-bold text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="text-sm">
                        {currentItems.map((art) => {
                          const questionsSet = [art.q1, art.q2, art.q3, art.q4, art.q5].filter(q => q && q.eng && q.eng.question).length;
                          return (
                            <tr key={art.id} className="hover:bg-gray-50 transition-colors group">
                              <td className="p-4 border-b border-gray-100 font-serif font-bold text-[#381111]">{art.title?.eng || 'Untitled'}</td>
                ...
                {isLoadingFeedback ? (
                  ...
                  <div className="overflow-auto flex-1 rounded-xl border border-gray-200 hide-scrollbar shadow-inner relative">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                      <thead className="sticky top-0 bg-[#E0CCB6] z-10 shadow-sm">
                        <tr className="text-[#4A260F] text-sm uppercase tracking-wider">
                          <th className="p-4 border-b border-gray-300 font-bold">Date</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Visitor ID</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Rating</th>
                          <th className="p-4 border-b border-gray-300 font-bold">Comments</th>
                        </tr>
                      </thead>
                      <tbody className="text-sm">
                        {currentItems.map((item) => (
                          <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                            <td className="p-4 border-b border-gray-100 text-gray-500">{new Date(item.created_at).toLocaleDateString()}</td>
                            <td className="p-4 border-b border-gray-100 font-mono text-xs text-gray-400">{item.visitor_id || 'Anonymous'}</td>
                            <td className="p-4 border-b border-gray-100 font-bold text-yellow-500">{item.rating ? `${item.rating} / 5` : 'N/A'}</td>
                            <td className="p-4 border-b border-gray-100 text-gray-600">{item.comments || 'No comment provided.'}</td>
                          </tr>
                        ))}
                        {currentItems.length === 0 && (
                          <tr>
                            <td colSpan="4" className="p-8 text-center text-gray-500 italic">No feedback entries yet.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
            ...
            <div className="flex justify-center items-center gap-2 mt-4 flex-shrink-0">
              ...
              <div className="flex gap-1">
                {Array.from({ length: totalPages }, (_, i) => (
                  <button key={i + 1} onClick={() => paginate(i + 1)} className={`w-8 h-8 rounded flex items-center justify-center font-bold shadow-sm transition-all ${currentPage === i + 1 ? 'bg-[#E19B2D] text-white' : 'bg-white text-[#381111] hover:bg-[#E0CCB6]'}`}>{i + 1}</button>
                ))}
              </div>
              <button onClick={() => paginate(currentPage + 1)} disabled={currentPage === totalPages} className="px-3 py-1 rounded bg-[#381111] text-[#E0CCB6] disabled:opacity-50 hover:brightness-110 shadow-sm transition-all">Next</button>
            </div>
          )}
          ...
          <div className="bg-[#F5EAD4] rounded-2xl w-full max-w-4xl max-h-[90dvh] flex flex-col shadow-2xl border-4 border-[#381111] overflow-hidden">
            ...
            <div className="bg-[#381111] px-6 py-4 flex justify-between items-center text-[#E0CCB6] flex-shrink-0">
              <h2 className="text-2xl font-serif font-bold">
                {modalMode === 'content' && (isNewItem ? 'Add New Painting' : 'Edit Painting Content')}
                {modalMode === 'translations' && 'Edit Translations'}
                {modalMode === 'quizzes' && 'Edit Quizzes'}
                <span className="ml-2 opacity-70 text-lg">({isNewItem ? 'Draft' : (editItem.title?.eng || `ID: ${editItem.id}`)})</span>
              </h2>
              <button onClick={closeModal} className="text-[#E0CCB6] hover:text-white font-bold text-xl">✕</button>
            </div>
              ...
              {modalMode === 'content' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <input type="text" value={editItem.title?.eng || ''} onChange={(e) => handleJsonChange('title', 'eng', e.target.value)} className={inputStyles} />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="font-bold text-sm">English Artist</label>
                    <input type="text" value={editItem.artist?.eng || ''} onChange={(e) => handleJsonChange('artist', 'eng', e.target.value)} className={inputStyles} />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="font-bold text-sm">Artist Year</label>
                    <input type="text" value={editItem.artist_year || ''} onChange={(e) => setEditItem({...editItem, artist_year: e.target.value})} className={inputStyles} />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="font-bold text-sm">Map Zone (1, 2, 3, or 4)</label>
                    <input type="number" value={editItem.zone || ''} onChange={(e) => setEditItem({...editItem, zone: parseInt(e.target.value)})} className={inputStyles} />
                  </div>
                  <div className="flex flex-col gap-2 md:col-span-2">
                    <label className="font-bold text-sm">English Clues (For Map)</label>
                    <textarea value={editItem.clues?.eng || ''} onChange={(e) => handleJsonChange('clues', 'eng', e.target.value)} rows="3" className={inputStyles}></textarea>
                  </div>
                  <div className="flex flex-col gap-2 md:col-span-2">
                    <label className="font-bold text-sm">Thumbnail URL</label>
                    <input type="text" value={editItem.thumbnail_url || ''} onChange={(e) => setEditItem({...editItem, thumbnail_url: e.target.value})} className={inputStyles} />
                  </div>
                </div>
              )}
                ...
                <div className="flex flex-col gap-6">
                  <div className="flex gap-2 border-b border-[#381111]/20 pb-2">
                    {['tag', 'chi', 'jap', 'kor'].map(lang => (
                      ...
                      >
                        {lang === 'tag' ? 'Tagalog' : lang === 'chi' ? 'Chinese' : lang === 'jap' ? 'Japanese' : 'Korean'}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-4 rounded-xl border border-[#381111]/10">
                    <div className="flex flex-col gap-2">
                      <label className="font-bold text-sm flex justify-between">Title <span className="opacity-60">(EN: {editItem.title?.eng})</span></label>
                      <input type="text" value={editItem.title?.[translateLang] || ''} onChange={(e) => handleJsonChange('title', translateLang, e.target.value)} className={inputStyles} />
                    </div>
                    <div className="flex flex-col gap-2">
                      <label className="font-bold text-sm flex justify-between">Artist <span className="opacity-60">(EN: {editItem.artist?.eng})</span></label>
                      <input type="text" value={editItem.artist?.[translateLang] || ''} onChange={(e) => handleJsonChange('artist', translateLang, e.target.value)} className={inputStyles} />
                    </div>
                    <div className="flex flex-col gap-2 md:col-span-2">
                      <label className="font-bold text-sm flex justify-between">Clues <span className="opacity-60">(EN: {editItem.clues?.eng})</span></label>
                      <textarea value={editItem.clues?.[translateLang] || ''} onChange={(e) => handleJsonChange('clues', translateLang, e.target.value)} rows="2" className={inputStyles}></textarea>
                    </div>
                    <div className="flex flex-col gap-2 md:col-span-2">
                      <label className="font-bold text-sm">Artist Description</label>
                      <textarea value={editItem.artist_description?.[translateLang] || ''} onChange={(e) => handleJsonChange('artist_description', translateLang, e.target.value)} rows="3" className={inputStyles}></textarea>
                    </div>
                    <div className="flex flex-col gap-2 md:col-span-2">
                      <label className="font-bold text-sm">Art Elements</label>
                      <textarea value={editItem.art_element?.[translateLang] || ''} onChange={(e) => handleJsonChange('art_element', translateLang, e.target.value)} rows="3" className={inputStyles}></textarea>
                    </div>
                  </div>
                </div>
              )}
              ...
              {modalMode === 'quizzes' && (
                <div className="flex flex-col gap-8">
                  {['q1', 'q2', 'q3', 'q4', 'q5'].map((qNum, index) => {
                    const qData = editItem[qNum]?.eng || { question: "", choices: ["", "", "", ""], correct_index: 0 };
                    return (
                      <div key={qNum} className="bg-white p-4 rounded-xl border border-[#381111]/20">
                        <h4 className="font-bold text-lg mb-4 text-[#381111] border-b pb-2">Question {index + 1} (English)</h4>
            ...
            <div className="bg-[#E0CCB6] px-6 py-4 border-t border-[#381111]/20 flex justify-end gap-4 flex-shrink-0">
              <button onClick={closeModal} className="px-6 py-2 rounded-lg font-bold text-[#381111] hover:bg-[#381111]/10 transition-colors">Cancel</button>
              <button onClick={saveChanges} disabled={isSaving} className="px-6 py-2 rounded-lg font-bold bg-[#381111] text-[#E0CCB6] hover:brightness-110 disabled:opacity-50 transition-colors flex items-center gap-2">
